    ...
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex flex-wrap justify-center gap-6 mb-12">
            {trustSignals.map((signal, index) => (
              <div key={index} className="flex items-center text-slate-600">
                {signal.icon}
                <span className="ml-2 text-sm font-medium">{signal.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan, index) => (
              <div key={index} className={`bg-white rounded-2xl shadow-lg p-8 ${plan.popular ? 'ring-2 ring-emerald-600 ring-opacity-50 relative' : ''}`}>
                {plan.popular && (
                ...
                <div className="text-center mb-6">
                  <div className={`text-4xl font-bold mb-2 ${plan.popular ? 'text-emerald-600' : index === 0 ? 'text-blue-600' : 'text-purple-600'}`}>
                    {plan.staffLimit}
                  </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-slate-600 mb-8">
            Join hundreds of care providers who trust TeamCertify for their compliance needs.
          </p>
          <button
            onClick={() => navigate('/signup')}
