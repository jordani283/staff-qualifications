...
-- 1. Create a settings table for admin configuration (create this FIRST)
...
-- 2. Insert default admin email setting
INSERT INTO app_settings (key, value, description)
VALUES (
    'admin_email',
    'admin@teamcertify.com',
    'Administrator email for certification expiry notifications'
)
ON CONFLICT (key) DO NOTHING;

-- 3. Insert default app base URL setting
INSERT INTO app_settings (key, value, description)
VALUES (
    'app_base_url',
    'https://yourteamcertifyapp.com',
    'Base URL for the TeamCertify application'
)
ON CONFLICT (key) DO NOTHING;

-- 4. Create RLS policy for app_settings (admin only)
ALTER TABLE app_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Only service role can access app settings" ON app_settings
    FOR ALL TO service_role USING (true);

-- 5. NOW create function to get expiring certifications (after app_settings table exists)
...
AS $$
    SELECT 
        ...
        sc.expiry_date,
        COALESCE(p.company_name, 'TeamCertify') as company_name,
